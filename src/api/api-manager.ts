...
class ApiManager {
    ...
    public static getReservation(reservationId: string): Observable<Reservation> {
        const observable: Observable<Reservation> = ApiService.getReservation(reservationId).pipe(
            map(ret => {
    ...
    public static deleteReservation(reservationId: string): Observable<any> {
        const observable: Observable<Reservation> = ApiService.deleteReservation(reservationId);
        return observable;
    }

    public static getReservations(): Observable<Reservation[]> {
        const observable: Observable<Reservation[]> = ApiService.getReservations().pipe(
            map(ret => ApiManager.getFormattedReservations(ret.data))
        );
        return observable;
    }

    public static createReservation(reservation: Reservation): Observable<Reservation> {
        const data = {
            id: reservation.id,
        ...
        const observable = ApiService.postReservation(data).pipe(map(() => reservation));
        return observable;
    }

    public static updateReservation(reservation: Reservation): Observable<Reservation> {
        const data = {
            id: reservation.id,
            guests: reservation.guests,
            createTime: reservation.createTime,
        ...
        const observable = ApiService.putReservation(data).pipe(map(() => reservation));
        return observable;
    }

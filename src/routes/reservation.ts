...
router.get("/", query("reservationId").isString(), (req, res) => {
	console.log("\nGET /reservation");
	...
	if (!errors.isEmpty())
		return res.status(400).json({
			message: "Must include reservationId in query params",
			errors: errors.array(),
		});
...
router.post(
	...
	query("size").isFloat({
		min: Utils.MINIMUM_RESERVATION_SIZE,
		max: Utils.MAXIMUM_RESERVATION_SIZE,
	}),
	(req, res) => {
		console.log("\nPOST /reservation");
		...
		if (!errors.isEmpty())
			return res.status(400).json({
				message: "Must include datetime and size in query params",
				errors: errors.array(),
			});
			...
			next: (isAvailable) => {
				if (isAvailable === true) {
					ReservationManager.createReservation(staged).subscribe({
						next: () => {
							res.status(200).json({
								message: `Pending reservation ${staged.id} created`,
								reservationId: staged.id,
								isHoliday: staged.isHoliday(),
							});
						},
						error: (err) => {
...
router.post(
	...
	(req, res) => {
		...
		if (!errors.isEmpty())
			return res.status(400).json({
				message: "Must include reservationId and user in body",
				errors: errors.array(),
			});
		...
		ApiManager.getReservation(reservationId).subscribe({
			next: (reservation) => {
				if (reservation.status !== Utils.PENDING) {
					return res.status(400).json({
						message: `Reservation ${reservationId} is not pending`,
					});
				}
				...
				ApiManager.updateReservation(reservation).subscribe({
					next: () => {
						return res.status(200).json({
							message: `Reservation ${reservationId} is successfully booked`,
						});
					},
					error: (err) => {
						return res.status(500).json({
							message: `Reservation ${reservationId} failed booked`,
						});
					},
				});
			},
			error: (err) => {
				res.status(500).json({
					message: `Could not get reservation ${reservationId}`,
				});
			},
		});
	}
...
router.delete("/", query("reservationId").isString(), (req, res) => {
	console.log("\nDELETE /reservation");
